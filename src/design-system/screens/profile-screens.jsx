  ...
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',background:'var(--surface-app)'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'12px 16px 24px'}}>
        {isC?(
          ...
          <>
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',padding:'16px 0 20px'}}>
              ...
              <h2 style={{fontFamily:'var(--font-sans)',fontSize:20,fontWeight:700,color:'var(--text-primary)',margin:'12px 0 2px',letterSpacing:'-0.3px'}}>{profile.name}</h2>
              <p style={{fontFamily:'var(--font-sans)',fontSize:13,color:'var(--text-secondary)',margin:0}}>{profile.email}</p>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:20}}>
              <SMini label="Silos" value={siloCount}/>
              <SMini label="Lanzas" value={tot}/>
              <SMini label="Alertas" value={0} color="var(--status-ok)"/>
            </div>
            <SCard label="MI CUENTA">
              <MRow icon="user"    label="Datos personales" onClick={()=>nav('edit-profile')}/>
              <MRow icon="map-pin" label="Mi campo" value={profile.farm.location.split(',')[0].trim()} onClick={()=>nav('edit-profile')} last/>
            </SCard>
            <SCard label="CONFIGURACIÓN">
              <MRow icon="wifi"   label="Mis lanzas"      value={`${onl}/${tot}`} onClick={()=>nav('devices')}/>
              ...
              <MRow icon="lock"   label="Cambiar contraseña" onClick={()=>{}}/>
              <MRow icon="refresh-cw" label="Repetir tutorial" onClick={()=>{}} last/>
            </SCard>
            <SCard label="SOPORTE">
              <MRow icon="message-circle" label="WhatsApp" onClick={()=>window.open('https://wa.me/5491100000000')}/>
              <MRow icon="file-text"      label="Legal"    onClick={()=>{}}/>
              <MRow icon="info"           label="Versión"  value="1.0.0" last/>
            </SCard>
          </>
        ):(
          /* ─── COMPACTO ─── */
        ...
        {!isC&&<p style={{fontFamily:'var(--font-sans)',fontSize:11,color:'var(--text-muted)',textAlign:'center',margin:'12px 0 0'}}>SiloGuard v1.0.0</p>}
      </div>
