> **Nota de archivo (2026-08-17):** este es el contenido íntegro del `CLAUDE.md` original del
> proyecto, recuperado con `git show HEAD:CLAUDE.md` desde el repo padre (`~/MiProyectoFigma`,
> donde arrancó el proyecto antes de mudarse a esta carpeta `SiloGuard/`). Se conserva como
> registro histórico — la parte de Design System/backend ya fue rescatada en su momento en
> `ARQUITECTURA.md` (commit `df7d96a`); lo único que no sobrevivió a esa migración fue la
> sección final "Estado del equipo y próximos pasos", con notas de trabajo internas de julio
> de 2026 (backend duplicado con un compañero, checklist de pantallas pendientes, etc.) que se
> reproducen acá tal cual, sin actualizar. No usar como fuente de verdad del estado actual del
> proyecto — para eso ver `ARQUITECTURA.md`, `docs/DEFENSA.md` y `BACKLOG.md`.
